...
#### TemplateFrame.Demo.Excel.AutoMapping
- 内容与 Excel 手写映射版一致；控制台打印每个元素的 `DataPath`；无手写映射方法。

#### TemplateFrame.Demo.Excel.Simple
...
- i18n（中英表头 + 定义名回读）见独立 Demo `TemplateFrame.Demo.Excel.Simple.I18n`。
