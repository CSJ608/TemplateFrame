...
6. `TemplateFrame.slnx` 改为**解决方案文件夹**：`src/` / `test/` / `samples/` 三组（项目变多后按源码/测试/Demo 归类）

### 验收
- `dotnet build TemplateFrame.slnx && dotnet test` 全绿（基础包 37 + Simple 17 用例，共 140）；`dotnet run --project samples/TemplateFrame.Demo.Excel.Simple` 依次输出 模板 / 填充后 / 回读（强类型）；`dotnet run --project samples/TemplateFrame.Demo.Word.AutoMapping` 依次输出 模板 / 校验 / 收货前 / 收货后 / 回读（强类型）
