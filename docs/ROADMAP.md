...
> 文末保留历轮启动指令供回顾；开展新工作时按当前范围编写指令，完成后更新状态入口与 CHANGELOG。

## 当前状态入口

- **2.4.1**：文档、注释和包描述修订见 [CHANGELOG](../CHANGELOG.md#241---2026-09-08)；D1、D2 已验收，发布验证及结果以[评审台账](reviews/2026-09-07-review.md)中的 2.4.1 记录为准。
- **2.4.0**：2026-09-07 发布内容见 [CHANGELOG](../CHANGELOG.md#240---2026-09-07)；实际发布及四包核验结论见[评审台账](reviews/2026-09-07-review.md)中 2026-09-08 的协调会话复审记录。本页引用已有核验结果，不视为本次重新执行发布验证。
- **R1～R5**：诊断、业务映射、生成并发、缓存生命周期与 Excel 回读索引整改均已由协调会话验收通过，状态及证据以[台账“必须复审的问题”](reviews/2026-09-07-review.md#必须复审的问题)为准；[性能证据及限制](PERFORMANCE.md)继续保留。
- 真实 Office/WPS 样本、负载/峰值内存和公开 API 兼容性门禁仍是台账中的后续观察项，不因本轮整改或发布自动完成。

下文是历史迭代档案：版本、日期、测试结果和启动指令反映当时状态，不作为当前操作指令；日常发布使用 [PUBLISHING](PUBLISHING.md)。
...
| **21** | **真实场景正确性收尾：嵌套 SDT / 0 行占位 / 图片锚点平移 / 数值精度 / docPr 唯一（2.3.1）** | ✅ 已完成（见下） |
...
> **状态**：✅ 已完成（2026-08-27，随 **2.3.1** 发布）。327 用例 × 2 TFM 全绿；`dotnet format --verify-no-changes` 通过。
