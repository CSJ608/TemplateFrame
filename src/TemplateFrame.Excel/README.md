...
- **引擎 ParseDetailed（2.3.0）**：导入方向对称出口——值转换失败的字段保留原始文本，并以 `ConversionFailed`（Warning，表格列带工作表行号）随 `TemplateParseResult` 返回；null 仍专指未填充，`Parse` 行为不变。
...
- 结构化转换诊断提供 `TableKey`、从 1 开始的数据行号 `DataRowNumber`、集合索引从 0 开始的 `DataPath`、失败输入 `RawValue` 和字符串 `TargetType`（默认 JSON 可序列化的类型描述）；Excel 原消息中的行号仍为工作表绝对行号。
...
- 历史性能快照（2026-08-24，仅描述当时样本，不保证线性伸缩或当前版本耗时）：千行明细填充 ~60ms、回读 ~115ms、构建 ~1ms；快照见仓库 `docs/PERFORMANCE.md`，基准项目 `test/TemplateFrame.Benchmarks`。
...
本轮 Excel 回读索引优化及同环境三档比较见仓库 `docs/PERFORMANCE.md` 的 R5 章节；旧快照不能用作新版本耗时承诺，累计托管分配也不是峰值内存。
