...
| 组件 | 职责 |
|---|---|
| `ExcelTemplateBuilder` | 组装带命名区域的 .xlsx：页面设置、列宽、单元格格式、合并单元格、表格（表头 + 示例行）、图片（单元格锚定） |
...
public sealed class DeliveryOrderExcelTemplateService : TemplateService<DeliveryOrderData, ExcelTemplateBuilder>
{
    ...
    protected override void BuildInitialTemplate()
    {
        ...
        Builder.AddText("A1", "送 货 单", new TextFormat { FontName = "黑体", SizePt = 16, Bold = true, Alignment = TextAlignment.Center });
...
见仓库 `samples/TemplateFrame.Demo.Excel` 的**送货单 Excel 版**（复用送货单数据，A5 横版）：
