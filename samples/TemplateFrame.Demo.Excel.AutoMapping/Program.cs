...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        var dir = args.Length > 0
            ? args[0]
            : Path.Combine(Path.GetTempPath(), "TemplateFrame.Demo.Excel.AutoMapping");
        Directory.CreateDirectory(dir);
        var templatePath = Path.Combine(dir, "Excel-AutoMapping-DeliveryOrder-template.xlsx");
        var prePath = Path.Combine(dir, "Excel-AutoMapping-DeliveryOrder-pre.xlsx");
        var postPath = Path.Combine(dir, "Excel-AutoMapping-DeliveryOrder-post.xlsx");
        ...
        // [5] 回读数据：读取已填充的 Excel 模板 → service.Parse → 强类型 DeliveryOrderData（自动映射反向）
        Console.WriteLine("\n[5] 回读数据：读取已填充的 Excel 模板 → service.Parse → 强类型 DeliveryOrderData（自动映射反向）");
