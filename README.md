...
| **Word 单据打印**（页眉页脚 / 表格 / 图片 / A5 横版） | `TemplateFrame.Word` | 内容控件（SDT）定位，改样式时须保留控件与表格结构 |
...
插件直接依赖 `DocumentFormat.OpenXml`，net462 资产另引用 `System.ValueTuple`；核心包不引用 OpenXML。
...
```csharp
var data = new MaterialsData { Items = [new MaterialLine { Code = "M001", Name = "螺栓", Unit = "个" }] };
```
...
- **校验与诊断**：Word/Excel 用 `FillDetailed` / `ParseDetailed` 获取告警；引擎转换失败保留原文，默认强类型映射保留属性默认值。完整说明见[校验与解析规则](docs/DESIGN.md#parse-diagnostics)。Simple 使用独立的 `Validate` / `Parse`，见[格式差异](docs/DESIGN.md#format-differences)。
- **服务生命周期**：Word/Excel 模板生成按实例串行，不代表整个服务线程安全；共享实例前请阅读[生命周期规则](docs/DESIGN.md#service-lifetime)。

## Word / Excel 灵活版式

单据打印这类复杂版式用 `TemplateService<TData, TBuilder>`——继承时声明插件构建器类型，版式能力就是构建器的方法。以下为结构片段，省略 DTO、契约与页眉回调；可运行项目见下方示例：
...
- Word 用 SDT tag，Excel 用命名区域（`TF_<Key>`）；编辑时须保留标记、有效引用与表格结构，每次填充都从未填充的原始模板开始。详见[格式规则](docs/DESIGN.md#format-differences)。
