...
### 文档
- 明确 Excel 行平移的引用、区域和绘图标记边界；精简公共 API 英文摘要，将映射、诊断和资源边界放入中文 remarks，保留必要原因。本版不改变运行时逻辑或 API 签名。
- 修正 Excel.Simple 包描述，准确说明命名区域和列定义名定位能力。
- 发布指南改用待发布版本变量，明确 Version / CHANGELOG / tag 一致性、首次配置与日常发布，并补齐四包及符号包验证。
- DESIGN 集中维护服务、诊断和格式规则，移除重复版本/迭代列表；根目录及三插件中英文 README 精简重复说明，修正 API 示例与格式边界、补齐链接。
- ROADMAP 增加 2.4.0 和 R1～R5 状态入口；保留历史迭代数据、评审记录及性能证据，文档交付见[评审台账](docs/reviews/2026-09-07-review.md)。

## [2.4.0] - 2026-09-07

### 修复与兼容性
- **详细解析诊断（R1）**：服务层默认自动映射的转换失败现在进入 `ConversionFailed` 告警，包含字段/表格、数据行号、属性路径、输入值和目标类型名称；同一失败与引擎告警合并时保留原消息。引擎 `FillData` 保留失败原文，强类型属性保持默认值，普通强类型 `Parse` 仍严格抛错。
- **业务映射（R2）**：`ParseDetailed` 继续执行直接或继承的 `MapFromData` 重写；显式 `MapFromDataDetailed` 重写仍优先。已有业务无需增加重写方法，但此前被绕过的业务逻辑和异常现在会生效，业务调用 `base.MapFromData` 时仍使用严格转换。
- **生成并发（R3）**：同一服务实例的 `BuildInitialTemplateFile` 从创建 Builder 到释放全程串行化，异常后可继续生成；版式/保存/释放阶段的同线程递归生成抛出 `InvalidOperationException`。公开及 protected 方法签名不变；同实例吞吐受串行化约束，回调不得等待同实例另一生成调用，不代表其他服务方法或业务状态均线程安全。
- **映射缓存生命周期（R4）**：缓存改为按契约引用身份弱关联，失去外部引用的契约及映射可由 GC 回收；相等但不同实例的契约分别缓存。活跃契约继续复用按 DTO 类型隔离的映射，首次并发访问允许重复构建；无容量或即时回收保证，首次映射后仍不得原地修改契约集合。
- `TemplateValidationIssue` 新增可选 `TableKey`、`DataRowNumber`、`DataPath`、`RawValue`、`TargetType`；目标类型为字符串，内置诊断可用默认 System.Text.Json 序列化。消费方需留意新增 JSON 字段、告警数量和 record 相等比较变化；不保证任意业务自定义 `RawValue` 均可序列化。未新增产品依赖；四包统一发布为 2.4.0。

### 性能
- **Excel 回读（R5）**：单次解析复用工作表行和单元格索引，消除逐格从头扫描行，保持稀疏行、缺失格、多列起点及首匹配语义。访问的工作表实际行数为 S、被访问行单元格总数为 K、查找次数为 Q 时，索引及查找期望成本为 O(S+K+Q)，额外空间 O(S+K)，不代表完整解析严格线性。
- 同机六列表格的 5000 行短基准均值由 1690.69 ms 降至 199.347 ms，累计托管分配由 46.86 MiB/op 降至 39.47 MiB/op；仅描述该样本，不保证提速倍数或峰值内存。环境、三档数据、原始证据和局限见 [性能文档](docs/PERFORMANCE.md)。

### 文档与验证
- 同步中英文 README、插件说明、服务生命周期与诊断设计，撤回历史快照的普遍线性伸缩承诺；新增映射、JSON、并发、缓存生命周期和 Excel 查找回归及三档回读基准。整改与交付验收记录见 [评审台账](docs/reviews/2026-09-07-review.md)。
